...
public class RocketManager : MonoBehaviour
{
    ...
    {
        if (transform.parent.GetComponent<PlanetManager>().IsPlayer())
        {
            rocketPlayer.SetActive(true);
            rocketEnemy.SetActive(false);
        } else
        { 
            rocketPlayer.SetActive(false);
            rocketEnemy.SetActive(true);
        }
    ...
    { 
        if (isFlying)
        { 
            Vector3 curDir = (transform.GetChild(0).position - transform.position); 

            float angle = Vector3.SignedAngle(curDir, rigidBody.velocity, transform.forward);
            transform.RotateAround(transform.position, transform.forward, angle);

            rigidBody.AddForce((transform.GetChild(0).position - transform.position).normalized * acceleration, ForceMode.Acceleration);
